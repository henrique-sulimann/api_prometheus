...
async function mid(req: express.Request, res: express.Response, next: express.NextFunction) {
...
app.get('/summary', mid, async (req, res) => {
    const buckets = await s3.listBuckets().promise()
    res.json({
        mensagem: "summary metric, com interceptor para buscar a porcentágel de latência",
        buckets: buckets.Buckets
    })
})
